...
<div class="my-3 rounded-2xl p-4" style="background: var(--quip-input-bg); border: 1px solid var(--quip-border-strong)">
  <div class="flex items-center gap-2 mb-3">
    <svg class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: #a78bfa">
  ...
  {#if plan.questions?.length}
    ...
    <ul class="space-y-1 mb-3">
      {#each plan.questions as q}
        <li class="flex items-start gap-2 text-[13px]" style="color: var(--quip-text-dim)">
          <span class="mt-1.5 w-1.5 h-1.5 flex-shrink-0 rounded-full" style="background: var(--quip-text-muted); opacity: 0.4"></span>
          <span>{q}</span>
        </li>
      {/each}
    </ul>
